...
class TernaryNode<T> {
    ...
    public boolean isLeaf() {
        return (children[0] == null) && (children[2] == null) && (children[1]==null);
    }
    ...
    private int getHeight(TernaryNode<T> node) {
        ...
        if (node != null){
            height = 1+Math.max(getHeight(node.getMiddleChild()),
                    (Math.max(getHeight(node.getLeftChild()),getHeight(node.getRightChild()))));
        }
